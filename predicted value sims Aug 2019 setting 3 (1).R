...
# Third setting:
# There is a strong association between Exposure and true Y, but the relationship between Exposure and Ypred is null--even though there are no confounders
...
# Generate auxiliary data 
	...
	#Generate exposure independent of covariates--so there is no confounding
		Exposure<-rnorm(N,55,7)


	#Generate Y based on covariates above, and exposure
       Error<-rnorm(N,2,1)  
	   Predictor<-White+Black+Poverty+Homeowner+NO2/max(NO2)+PM25/max(PM25)+Old+Female+
	   				Population/max(Population)+ 10*Exposure/max(Exposure)
...
# plot for true Y
	...
	hyp_newdata2c <- within(hyp_newdata2c, {
		...
		#Save plot
		tiff(filename = "/Users/joancasey/Documents/Columbia/RMF/Urban_Institute/Scenario3_Pred_True_Y.tiff", type = "cairo" ,  res=300, height = 5, width = 6, units = "in")
